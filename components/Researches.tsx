          ...
          <div className="min-w-full flex-shrink-0 p-2 sm:p-8 lg:p-16">
            <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6 lg:p-8 max-w-xs sm:max-w-sm lg:max-w-3xl mx-auto">
              <div className="relative h-48 w-full mb-4">
                <Image src={researchesData.image} alt={researchesData.title} layout="fill" objectFit="cover" className="rounded-lg" />
              </div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2 text-center">{researchesData.title}</h3>
              <p className="text-gray-700 mb-4 text-center">{researchesData.description}</p>
              <div className="flex flex-col sm:flex-row justify-center items-center space-x-0 sm:space-x-4 space-y-2 sm:space-y-0">
                <Link href={researchesData.paperLink} target='_blank'>
                  <span className="inline-flex items-center bg-blue-600 text-white py-2 px-8 rounded-md text-lg font-medium hover:bg-blue-700 transition duration-300 ease-in-out w-full sm:w-auto text-center justify-center">
                    <FileText className="h-5 w-5 mr-2" />
                    Paper
                  </span>
                </Link>
                <Link href={researchesData.repoLink} target='_blank'>
                  <span className="inline-flex items-center bg-purple-600 text-white py-2 px-8 rounded-md text-lg font-medium hover:bg-purple-700 transition duration-300 ease-in-out w-full sm:w-auto text-center justify-center">
                    <Github className="h-5 w-5 mr-2" />
                    Repository
                  </span>
                </Link>
                <Link href={researchesData.videoLink} target='_blank'>
                  <span className="inline-flex items-center bg-red-600 text-white py-2 px-8 rounded-md text-lg font-medium hover:bg-red-700 transition duration-300 ease-in-out w-full sm:w-auto text-center justify-center">
